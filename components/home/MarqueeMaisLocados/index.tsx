...
const firstRow = quartos.slice(0, quartos.length / 2);
const secondRow = quartos.slice(quartos.length / 2);

const QuartoCard = ({ img, preco, descricao, camas, cidade, pais }) => {
  return (
    <figure
      className={cn(
        ...
        "flex flex-col justify-between h-72" // Aumenta a altura e organiza o conteúdo
      )}
    >
      <img
      ...
      <figcaption className="flex flex-col justify-between mt-3 h-full">
        <p className="text-lg font-semibold text-primary">{cidade}, {pais}</p>
    ...
    <div className="relative flex w-full flex-col items-center justify-center overflow-hidden rounded-lg bg-background">
      <div className="flex flex-row items-center my-4">
        <div className="w-[50svw] text-right pr-2 ml-24">
          <WordRotate className="text-4xl font-bold text-black dark:text-white whitespace-nowrap" words={["Nossos Quartos Mais"]} />
        </div>
      ...
      <div className="h-[80svh]">
        <Marquee pauseOnHover className="[--duration:20s]">
          {firstRow.map((quarto, index) => (
            <QuartoCard key={index} {...quarto} />
          ))}
        </Marquee>
        <Marquee reverse pauseOnHover className="[--duration:20s]">
          {secondRow.map((quarto, index) => (
            <QuartoCard key={index} {...quarto} />
          ))}
        </Marquee>
